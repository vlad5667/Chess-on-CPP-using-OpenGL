	...
	void Scene::on_mouse(int button, int state, int x, int y)
	{
		...
		if (state == GLUT_UP) { // кнопка віджата
			...
			if (button == 0) {
				...
				//TODO если король бьёт фигуру, проверять чтобы он не попадал при этом под новый шах
				...
				if (mouseXCell == -1 || mouseZCell == -1) {
					...
					if (fields[currentZ][currentX] != -1 && (zStart != mouseZCell || xStart != mouseXCell)) {
						...
						if (checkW != -1 && whiteMove) {
							...
							// Якщо фігура не намагається нанести удар по фігурі, а просто намагається походити, перевіряємо коректність ходу
						...
						// Якщо чорним оголошено шах і зараз хід чорних
							...
							//TODO поменять возврат isCHeckOccured обратно на bool. И кста проделать такую же проверку двойного шаха с белыми фигурами.
							// Ещё сделать коммит после этого на гит.
							// Якщо сталая колізія з іншою фігурою, то перевіряємо чи можливий удар
							if (collision && fields[mouseZCell][mouseXCell] != -1 && pieces[fields[mouseZCell][mouseXCell]] != p) {
								if (hit = p->isHitPossible(pieces, fields, zStart, xStart, mouseZCell, mouseXCell, currentZ, currentX)) {
									int k = fields[mouseZCell][mouseXCell];
							...
							// Якщо фігура намагається забрати фігуру, що не спричинала шах, то цей удар неможливий, тому він скасовується
							else if (p->correctMove(pieces, fields, zStart, xStart, mouseZCell, mouseXCell)) {
								// Якщо після цього ходу король не звільнився від шаху, повертаємо фігуру назад
